...
Exposes three tools to Claude Code (and any MCP-compatible agent) via stdio transport:
...
Verify in Claude Code: run /mcp — devmemory should appear as connected with 3 tools.

Note: directory is named mcp_server/ (not mcp/) to avoid shadowing the mcp PyPI package.
"""

from mcp.server.fastmcp import FastMCP
from mcp_server.tools import search_memories, build_context, remember_memory
